...
import { FiMapPin, FiPhone, FiRefreshCcw, FiUser } from "react-icons/fi";
                        ...
                        {orders.length === 0 ? (
                            ...
                            orders.map(order => (
                                <tr key={order.id} className="hover:bg-gray-50 transition">
                                    ...
                                    <td className="p-3 border-b border-gray-200 text-center">CUS{order.customerId}</td>
                                    ...
                                    <td className="text-center border-b border-gray-200 relative group status-dropdown">
                                        ...
                                        {editingOrderId === order.id && allowedTransitions[order.statusName]?.length > 0 && (
                                            <div
                                                className="absolute left-1/2 -translate-x-1/2 mt-2 bg-white border border-gray-200 shadow-lg z-50 text-left w-max rounded-lg overflow-hidden"
                                    ...
                                    <td className="text-center border-b border-gray-200">
                                        {order.paymentMethod === "VNPAY" ? (
                                            <span className={`px-3 py-2 rounded text-white font-semibold ${getPaymentStatusClass(order.paymentStatus)}`}>
                                                {getPaymentStatusLabel(order.paymentStatus)}
                                            </span>
                                        ) : (
                                            <span className="px-3 py-2 border rounded bg-gray-50 font-semibold text-gray-800">
                                                COD
                                            </span>
                                        )}
                                    </td>
                ...
                <div onClick={() => setIsDetailOpen(false)}
                    ...
                    <div onClick={(e) => e.stopPropagation()}
                        ...
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 border-b pb-4 mb-4">
                            <div className="flex flex-col justify-between gap-2">
                                ...
                                <div className="flex flex-col gap-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`px-3 py-1 rounded font-semibold ${statusBgColor(orderDetails.statusName)}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {orderDetails.paymentMethod === "VNPAY" && (
                                            <span
                                                className={`px-3 py-1 text-white rounded font-semibold ${getPaymentStatusClass(orderDetails.paymentStatus)}`}
                                            >
                            ...
                            <div className="text-gray-600 text-sm pt-5 ">
                            ...
                            {orderDetails.items.map(item => (
                                <div key={item.id} className="flex items-center bg-gray-100 p-3 rounded-lg">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-medium text-gray-800">{item.variantName}</p>
                                        <p className="text-gray-600 text-sm mt-1">x{item.quantity}</p>
                                    </div>
